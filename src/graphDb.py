...
def creatingGroups(c, nGroups):
    ...
    groups = range(minFollowers, maxFollowers,
                   (maxFollowers-minFollowers) / nGroups)
...
if __name__ == "__main__":
    ...
    rtsPerTweets = [float(rt) / tweet if tweet != 0 else 0
                    for rt, tweet in zip(rts, tweets)]
    favsPerTweets = [float(fav) / tweet if tweet != 0 else 0
                     for fav, tweet in zip(favs, tweets)]

    fig, ax = plt.subplots()
    # Minimum 7 to avoid seeing the points of 0
    # retweets and a little bit more in the max
    ax.set_ylim([7, max(rtsPerTweets)+100])
    ax.plot(groups[1:], rtsPerTweets, marker='o',
            linestyle='None', color='green')
    ax.plot(groups[1:], favsPerTweets, marker='o',
            linestyle='None', color='orange')
    plt.xlabel("Number of followers")
    plt.ylabel("Average of interactions (rts or favs / amount of tweets")
    plt.show()
